...
class ProfitLostAccountCrudController extends CrudController
{
            ...
            ->where(function($query) use ($filter_year){
                ...
                ->orWhere(function($q) use ($filter_year){
                    ...
                    ->where(function($q2) use ($filter_year){
                        $q2->whereExists(function ($sub) use ($filter_year) {
                            $sub->select(DB::raw(1))
                                ->from('invoice_clients')
                                ->whereColumn('invoice_clients.client_po_id', 'project_profit_lost.client_po_id')
                                ->whereYear('invoice_clients.invoice_date', $filter_year);
                        })
    ...
    public function total_detail_project($id, $pure = 0)
    {
        ...
        if(!empty($filter['filter_year']) && $filter['filter_year'] != 'all'){
            ...
            //     }
            // })
        $profit_lost_all_price = $profit_lost_all_price->first();
        ...
        if ($request->has('type')) {
            if ($request->type == 'project') {
                ...
                CRUD::column([
                    ...
                    'function' => function ($entry) use ($settings) {
                        $currency = ($settings?->currency_symbol) ? $settings->currency_symbol : "Rp.";
                        return $currency . number_format($entry->job_value_include_ppn_logic, 2, ",", ".");
                    },
                    'orderLogic' => function ($query, $column, $columnDirection) {
        ...
        if ($request->has('type')) {
            if ($request->type == 'project') {
                ...
                CRUD::column([
                    ...
                    'type'  => 'closure',
                    'function' => function ($entry) use ($status_file) {
                        return $this->priceFormatExport($status_file, $entry->job_value_include_ppn_logic);
                    },
                    'orderLogic' => function ($query, $column, $columnDirection) {
                        $query->orderBy('client_po.job_value_include_ppn_logic', $columnDirection);
                    }
