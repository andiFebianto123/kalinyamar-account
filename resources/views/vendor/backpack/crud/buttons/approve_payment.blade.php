...
@if ($user_id == $entry->approval_user_id && $entry->approval_status == 'Pending')
    <button
            ...
            data-title-approval="{{ trans('backpack::crud.voucher_payment.confirm.title') }}"
            data-body="{{ trans('backpack::crud.voucher_payment.confirm.confirm_approved_statement') }}"
            title="Approve"
        >
        Approve
    </button>
        ...
        $('#btn-approve').off('click').on('click', function(e){
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    swal("{{trans('backpack::crud.voucher_payment.confirm.alert_success')}}", "{{trans('backpack::crud.voucher_payment.confirm.confirm_after_success')}}", "success").then(() => {
                        if(response.events){
