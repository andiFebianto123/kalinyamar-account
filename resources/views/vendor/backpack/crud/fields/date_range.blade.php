...
<?php
    ...
    if (isset($entry)) {
        $start_value = formatDate($entry, $fieldName[0]);
        $end_value = formatDate($entry, $fieldName[1]);
    }
...
@include('crud::fields.inc.wrapper_start')
    <input class="datepicker-range-start" type="hidden" name="{{ $fieldName[0] }}" value="{{ old(square_brackets_to_dots($fieldName[0])) ?? $start_value ?? $start_default ?? '' }}">
    <input class="datepicker-range-end" type="hidden" name="{{ $fieldName[1] }}" value="{{ old(square_brackets_to_dots($fieldName[1])) ?? $end_value ?? $end_default ?? '' }}">
    ...
    <script>
        function bpFieldInitDateRangeElement(element) {
            ...
            setTimeout(() => {
                ...
                $visibleInput.off('keydown').on('keydown', function(e){
                    e.preventDefault();
                    return false;
                });
