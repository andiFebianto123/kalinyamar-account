...
@include('crud::fields.inc.wrapper_start')
    ...
    @if(isset($field['prefix']) || isset($field['suffix'])) <div class="input-group"> @endif
        ...
        <select
            ...
            @php
                $options = $field['options'] ?? function ($query) {
                    return $query->get();
                };

                $related_model = new $entity_model;
                $query = $related_model->newQuery();
                $results = $options($query);
            @endphp

            @foreach ($results as $entry)
                <option value="{{ $entry->getKey() }}" @if ((string) $entry->getKey() === (string) $selected_key) selected @endif>
